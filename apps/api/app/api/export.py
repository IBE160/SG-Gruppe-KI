...
async def request_data_export(
    ...
    try:
        ...
        user_profile_response = supabase.from("Users").select("*").eq("id", user_id).single().execute()
        user_goals_response = supabase.from("Goals").select("*").eq("user_id", user_id).execute()
        user_equipment_response = supabase.from("Equipment").select("*").eq("user_id", user_id).execute()
        user_workout_logs_response = supabase.from("WorkoutLogs").select("*").eq("user_id", user_id).execute()
