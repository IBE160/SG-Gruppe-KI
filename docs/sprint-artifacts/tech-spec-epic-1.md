...
1.  **New User Registration (Email):**
    ...
    7.  Frontend stores the JWT and navigates the user to the Onboarding Flow (Flow 2) – *temporarily redirects to `/onboarding-placeholder` until Story 1.3 is complete*.
...
8.  **AC1.2.5:** Upon successful authentication, new users are redirected to the Onboarding flow (Flow 2 – *temporarily redirects to `/onboarding-placeholder` until Story 1.3 is complete*), and returning users are redirected to the Dashboard.
